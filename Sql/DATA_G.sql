...
DROP PROCEDURE DATA_G.GET_ID_CIUDAD

IF OBJECT_ID('DATA_G.GET_ID_SERVICIO') IS NOT NULL
DROP PROCEDURE DATA_G.GET_ID_SERVICIO
...
GO
CREATE PROCEDURE DATA_G.GET_ID_CIUDAD(@CIUDAD NVARCHAR(255)) AS
BEGIN
...
GO
CREATE PROCEDURE DATA_G.GET_ID_SERVICIO(@SERVICIO NVARCHAR(255)) AS
BEGIN
	DECLARE @SERVICIO_ID INT
	
	SELECT @SERVICIO_ID = Descripcion FROM DATA_G.TIPODESERVICIO
...
GO
CREATE PROCEDURE DATA_G.CREAR_AERONAVE ( @FECHAALTA DATETIME, @MATRICULA NVARCHAR, @MODELO NVARCHAR, @KGDISP NUMERIC(18,0), @FABRICANTE NVARCHAR (255), @IDSERV INT, @CANTBUTACAS INT) AS
BEGIN 
